...
class CustomSalarySlip(SalarySlip):
	def get_component_totals(self, component_type, depends_on_payment_days=0):
		...
		for d in components:
			...
			frappe.log_error(
				f"Row: {d.salary_component} | Amt: {amount} | Curr: {row_currency} | Rate: {exchange_rate} | CompAmt: {d.amount_in_company_currency}",
				"Salary Slip MultiCurrency Debug",
			)
	...
	def set_totals(self):
		...
		frappe.log_error(
			f"Completed set_totals. Gross: {self.gross_pay}, Net: {self.net_pay}",
			"Salary Slip MultiCurrency Debug",
		)
	...
	def calculate_custom_leave_fields(self):
		if (
			not getattr(self, "employee", None)
			or not getattr(self, "start_date", None)
			or not getattr(self, "end_date", None)
		):
			return
		...
		""",
			(self.employee, self.end_date, self.start_date),
			as_dict=True,
		)
		...
		for leave in leave_applications:
			...
			if overlap_days > 0:
				leave_type_doc = frappe.db.get_value(
					"Leave Type",
					leave.leave_type,
					["include_holiday", "is_lwp", "skip_expenses"],
					as_dict=True,
				)
